...
// Write a stereo interleaved IEEE-float WAV. samplesL/samplesR same length.
// Used by the stereo_state_isolation regression test (qc_backlog.md).
export function writeStereoWav(path, samplesL, samplesR, sr = 48000) {
  if (samplesL.length !== samplesR.length)
    throw new Error('writeStereoWav: L/R length mismatch');
  ...
  for (let i = 0; i < N; i++) {
    buf.writeFloatLE(samplesL[i], 44 + i * 8);
    buf.writeFloatLE(samplesR[i], 44 + i * 8 + 4);
  }
  writeFileSync(path, buf);
}

// Split a (potentially stereo) readWav() result into separate L/R Float32Arrays.
export function deinterleaveStereo(readResult) {
  const { samples, channels } = readResult;
  if (channels === 1) return { L: samples, R: samples };
  if (channels !== 2) throw new Error(`deinterleaveStereo: expected 1 or 2 ch, got ${channels}`);
