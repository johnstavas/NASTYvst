// SmokeGainV0 editor implementation. Generated.
...
SmokeGainV0Editor::SmokeGainV0Editor(SmokeGainV0AudioProcessor& p)
    : juce::AudioProcessorEditor(&p), proc_(p)
{
    ...
    n_gain__gainDb_slider_.setSliderStyle(styleFor("slider"));
    n_gain__gainDb_slider_.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 60, 16);
    addAndMakeVisible(n_gain__gainDb_slider_);
    n_gain__gainDb_label_.setText("Gain", juce::dontSendNotification);
    n_gain__gainDb_label_.setJustificationType(juce::Justification::centred);
    addAndMakeVisible(n_gain__gainDb_label_);
    n_gain__gainDb_attach_ = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(
        proc_.apvts, "n_gain__gainDb", n_gain__gainDb_slider_);
...
void SmokeGainV0Editor::paint(juce::Graphics& g) {
    g.fillAll(juce::Colour::fromRGB(28, 30, 36));
    g.setColour(juce::Colour::fromRGB(220, 225, 230));
    g.setFont(16.0f);
    g.drawText("SmokeGainV0", getLocalBounds().removeFromTop(28), juce::Justification::centred);
}

void SmokeGainV0Editor::resized() {
    ...
    if (knobCount > 0) {
...

        {
            ...
            n_gain__gainDb_label_.setBounds(slot.removeFromTop(18));
            n_gain__gainDb_slider_.setBounds(slot.reduced(6));
            x += slotW;
        }
