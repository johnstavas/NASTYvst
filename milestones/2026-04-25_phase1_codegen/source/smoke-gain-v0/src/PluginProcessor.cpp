// SmokeGainV0 — generated PluginProcessor implementation.
...
SmokeGainV0AudioProcessor::makeLayout() {
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID { "n_gain__gainDb", 1 },
        "Gain",
...
SmokeGainV0AudioProcessor::SmokeGainV0AudioProcessor()
...
bool SmokeGainV0AudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const {
...
void SmokeGainV0AudioProcessor::prepareToPlay(double sr, int blockSize) {
    graph_.prepare(sr, blockSize);
    setLatencySamples(graph_.getLatency());
}

void SmokeGainV0AudioProcessor::releaseResources() {}

void SmokeGainV0AudioProcessor::reset() { graph_.reset(); }

void SmokeGainV0AudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&) {
    ...
    graph_.setParam("n_gain", "gainDb", apvts.getRawParameterValue("n_gain__gainDb")->load());
...
void SmokeGainV0AudioProcessor::processBlockBypassed(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&) {
    const int N      = buffer.getNumSamples();
    const int numOut = getTotalNumOutputChannels();
...
juce::AudioProcessorEditor* SmokeGainV0AudioProcessor::createEditor() {
    return new SmokeGainV0Editor(*this);
}

void SmokeGainV0AudioProcessor::getStateInformation(juce::MemoryBlock& destData) {
    auto state = apvts.copyState();
    std::unique_ptr<juce::XmlElement> xml(state.createXml());
    if (xml != nullptr) copyXmlToBinary(*xml, destData);
}

void SmokeGainV0AudioProcessor::setStateInformation(const void* data, int sizeInBytes) {
...
juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter() {
    return new SmokeGainV0AudioProcessor();
}
