...
**Updated:** 2026-04-26 (xformerSat + corpus-sweep + srcResampler + Tier-S Cluster A complete: optoCell + blackmerVCA + varMuTube + fetVVR + diodeBridgeGR — dynamics gap CLOSED)
...
| **Sandbox ops catalog** | **132/183 shipped (~72%); 46 parity-verified.** **Tier-S Character Cluster A COMPLETE 2026-04-26.** All 5 gain-reduction elements shipped: #141 optoCell (LA-2A) · #142 blackmerVCA (dbx/THAT 2180) · #145 varMuTube (Manley/670) · #147 fetVVR (UREI 1176) · **#179 diodeBridgeGR ✅+P** (Neve 33609/2254). **Dynamics gap CLOSED.** |
...
- **#179 diodeBridgeGR shipped ✅+P — 5/5 Tier-S Character Cluster A COMPLETE. Dynamics gap CLOSED.** Phenomenological diode-bridge GR cell modeling Neve 33609 / 2254 / 8014 family. Memoryless. Hill function (β=1.8 — sits between varMuTube's 1.5 and fetVVR's 2.0) + **PURE-ODD 3H distortion from cubic** (`distortion · comprDepth · x³ · gain`) — diode-bridge topology symmetry cancels even harmonics by design. Optional `asymmetry` knob adds 2H+4H for component-mismatch realism. **Critical model adjustment caught + fixed during ship:** initial `(x·gain)³` produced 3H/1H ratio that DROPPED with compression (wrong); fixed to `x³·gain` so ratio depends purely on comprDepth (matches real Neve behavior — distortion rises with GR). **Math-by-definition declared:** Neve service manuals + Ben Duncan paper + Wikipedia 33609/2254 articles all inaccessible at ship time. Topology = GMR JAES 2012; diode-bridge symmetry → odd-only is general analog-electronics knowledge. 21-test suite PASS including **the KEY signature test: pure-3H, no-2H, no-4H from topology symmetry** + comprDepth coupling. Native parity bit-identical at default. 6 deferred upgrade paths in research-debt — P1 verbatim Neve 2254 schematic + Duncan paper, P2 tanh saturation, P2 attack/release dynamics, P3 Vbe asymmetry.
- **#147 fetVVR shipped ✅+P** — **4th Tier-S character cluster A member.** Phenomenological JFET-VVR GR cell modeling UREI/UA 1176 family (2N3819 JFET, fast attack, aggressive harmonics). Memoryless. Sharper knee than varMuTube (β=2 default vs 1.5 — matches FET pinch-off curve steeper than vari-mu). **Distinguishing distortion model:** independent `distortion2H` (even, FET asymmetric channel conduction) + `distortion3H` (odd, FET pinch-off non-linearity) → mixed 2H+3H character. "All buttons in" reputation available by cranking both distortion params. **Math-by-definition declared:** 1176 service manual + 2N3819 datasheet inaccessible at ship time. Topology = GMR JAES 2012; FET ohmic-region equation captured from Wikipedia JFET article. 21-test suite PASS (gain curve, sharper-knee verification vs varMuTube, 2H-only / 3H-only / mixed spectral verification, "all buttons in" character verification, comprDepth coupling). Native parity bit-identical at default. 7 deferred upgrade paths in research-debt — P1 verbatim 1176 manual + 2N3819 datasheet, P2 measured curves + plateau onset, P2 dynamics absorption, P3 polarity asymmetry.
- **#145 varMuTube shipped ✅+P** — **3rd Tier-S character cluster A member.** Phenomenological variable-mu tube GR cell modeling Manley Variable Mu / Fairchild 670 / Altec 436 family. Memoryless. Soft-knee Hill function `gain = 1/(1 + (cv/V0)^β)` + even-symmetric distortion (DC + 2H + 4H) that scales with `(1 - gain)` — the canonical vari-mu signature: more compression → more 2H content. **Math-by-definition declared with honest gaps:** 6386/6BC8 datasheets, Pakarinen-Yeh CMJ 2009 PDF, Manley/670 service docs, and Norman Koren site ALL inaccessible during ship session (404/403/paywall). Topology anchor = GMR JAES 2012 §Soft Knee. Distortion-coupling principle = general tube physics (Langford-Smith Ch.13, RCA Tube Manual RC-30 — secondary sources). 21-test suite PASS (gain curve monotonicity, knee tunability, even-only spectrum 2H+4H/no 3H, vari-mu signature ratio verification, block-invariance, SR-invariance). Native parity bit-identical at default. 7 deferred upgrade paths in research-debt — verbatim datasheet curves (P1) + Pakarinen-Yeh full text (P1) lead the list.
- **#142 blackmerVCA shipped ✅+P** — **2nd Tier-S character cluster A member.** Log-add-antilog VCA per Blackmer US Patent 3,714,462 (Tier-S named-inventor patent). Memoryless. cv input = gain in dB (cv=0 → unity). `bias` param adds class-AB Vbe-mismatch character via `|y|` Fourier-decomposable distortion (DC + 2H + 4H — TRUE 2nd-harmonic content matching dbx character). Patent matching tolerance (1 mV / 40 µA) ≈ bias=0.025 → ~−40 dB 2H. **Critical bug caught + fixed during ship:** initial model used `bias·y·|y|` (sign-preserving) which produces ONLY ODD harmonics (3rd, 5th); switched to `bias·|y|` (sign-flipping) which produces EVEN harmonics matching real Blackmer character. Math verification of 2H/3H content via Goertzel DFT confirms even-only spectrum. 24-test math+stress suite PASS (gain accuracy ±50 dB across full patent range, true-2H verification, DC offset verification, block-invariance, memoryless integrity). Golden `1dd8f45d00ac1708…`. Native parity **bit-identical** (0.000e+0 across all 5 canon signals — pure linear multiplier at default cv=0). 5 deferred upgrade paths in research-debt: P2 measured 2H curve, P3 control feedthrough, P3 control bandwidth, P3 self-noise, recipe-rule for DC blocking.
- **#141 optoCell shipped ✅+P** — **First Tier-S character cluster A member.** Phenomenological LA-2A T4-style optical-isolator GR cell. Two-state envelope (envFast asymmetric attack/release per UA spec — 10 ms attack, 60 ms initial release; envSlow symmetric one-pole at 1–15 s program-dependent tau). Effective env = max(envFast, envSlow); gain = 1/(1+responsivity·env²). **Math-by-definition declared** — no peer-reviewed T4 thermal-DSP model in literature; topology anchored to GMR JAES 2012 (Tier-A), parameter values anchored to UA blog T4 spec (Tier-B vendor-authored). 21-test math+stress suite PASS (UA T4 spec verification, program-dependent recovery, block-invariance, SR-invariance, denormal flush, state isolation). Golden `21207891…`. Native parity GREEN at −144.5 dB worst case. 4 deferred upgrade paths in research-debt: P1 validated thermal-coupling model, P2 tunable LDR exponent, P2 asymmetric warm-up/cool-down, P3 Felt 2010 paper. **Closes 1/5 of dynamics gap.** Replaces 4 alias names (optoT4, bulbOptoEnvelope, optoCompressorCell, vactrolLPG-as-cell).
...
|---|---|---|
| ✅+P | 46 | Shipped + parity-verified post-Phase-4 |
| ✅ (legacy) | 86 | Shipped pre-Phase-4 (rebadge to ✅+P as touched) |
| 🚧 | ~5 | Registry-only (no sidecar yet) |
| ⬜ | ~44 | Not started — pre-sweep: #119 hrtf · #120/121/122 ML · #140 pultecEQ · reserved slots (~6). +43 from corpus-sweep, minus #166 + #141 + #142 + #145 + #147 + #179 shipped (full Tier-S Cluster A). |
...
- ~~**#166 srcResampler**~~ ✅+P shipped 2026-04-26 (cadence proof).
- ~~**#141 optoCell**~~ ✅+P shipped 2026-04-26 (1st Tier-S character cluster A — LA-2A T4).
- ~~**#142 blackmerVCA**~~ ✅+P shipped 2026-04-26 (2nd Tier-S character cluster A — dbx/THAT 2180).
- ~~**#145 varMuTube**~~ ✅+P shipped 2026-04-26 (3rd Tier-S character cluster A — Manley Vari-Mu/Fairchild 670).
- ~~**#147 fetVVR**~~ ✅+P shipped 2026-04-26 (4th Tier-S character cluster A — UREI 1176).
- ~~**#179 diodeBridgeGR**~~ ✅+P shipped 2026-04-26 (**5th and FINAL Tier-S character cluster A — Neve 33609/2254 — DYNAMICS GAP CLOSED**).
- **#140 pultecEQ** — Pultec EQP-1A manual. Parallel LCR + tube + UTC OT character. Depends on #139 (now shipped).
- **Tier-S Character Cluster B (tube preamp atoms — 5 ops)** queued: #148 triodeStage · #154 psuRectifierSag · #155 pushPullPower · #156 phaseInverter · #157 cathodeBiasShift. Ships as a coherent set; #113 tubeSim deprecates to alias of triodeStage-with-defaults once #148 lands.
