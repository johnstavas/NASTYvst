...
| 184 | pingPong | 🚧 | **Equal-level stereo ping-pong delay (added 2026-04-29)**. Single mono buffer + two staggered linear-interp read taps: R reads at -TIME, L reads at -2·TIME, so a pulse fires R, L, R, L alternating at perfectly balanced amplitude per bounce-pair. Built-in one-pole LP tone filter inside the FB loop (LP-filtered long-tap fed back to write input) for progressive darkening + frequency-dependent loop loss bounding stability for fb<1. SPREAD knob blends between full L/R panning (1.0) and mono-summed wet (0.0) for in-the-mix bouncing. Per-sample one-pole smoothing on delay length (50 ms target) kills knob-zipper. Linear-interpolated fractional read kills boundary pops at integer-sample wrap. Soft clamp on FB tap at ±1.5 as paranoid safety. **PRIMARY**: Zölzer DAFX 2nd ed. §3.1 "Basic Delay Structures" Fig 3.7 + JOS PASP "Delay Lines" (Lagrange interp read, simplified to 1st order). Declared deviation: classic Roland Space Echo / dub ping-pong uses two cross-coupled buffers with input on one side only — that topology is asymmetric (one side 2× louder per tap). We trade input-side-first character for clean L/R balance via the single-buffer two-tap form. Params: time/feedback/tone/spread/mix. 13 math tests green (silence-in/silence-out, mix=0 dry passthrough, pulse R/L tap timing, fb=0.85+bright stable, fb clamp engaged, tone darkens repeats vs bright, reset clears state, NaN/Inf clamps, time=10ms minimum, getLatencySamples=0, determinism, missing IO). Standalone worklet mono-collapses for headless tests; inline AudioWorkletProcessor in workletSources.js produces real 2-channel stereo output. nativeSkip flagged in per_op_specs (multi-output stereo needs stereo native harness). Latency=0. Powers PingPong brick (Stage-3 dogfood). |
...
|---|---|---|---|---|
| 139 | xformerSat | Character / Saturation | 🚧 (2026-04-26) | **De Paiva 2011** "Real-Time Audio Transformer Emulation for Virtual Analog Models" (EURASIP J. Adv. Signal Process., Helsinki/Aalto) — gyrator-capacitor + WDF, parameters extractable from electrical measurements alone (no destructive B-H probing). **Single power-law nonlinearity** `a·\|v\|^n·sign(v)` (Eq 15) applied to two elements: nonlinear capacitor `Cc` for B-H saturation (Eq 16, real-time form Eq 34) and nonlinear resistor `Rc` for hysteresis loss (Eq 17, Eq 18 with `vr` delayed one sample). Real-time cost ~10 mul + 1 LUT-pow per sample. Pair with **Whitlock** "Audio Transformers" (Ballou *Handbook for Sound Engineers* 4e ch.) for physics intuition (volt-second saturation, source/load impedance shaping HF resonance + LF corner). See `depaiva_transformer_emulation.md` + `whitlock_audio_transformers.md`. Five user-facing controls proposed: `drive`, `coreSize` (`a` in Eq 15), `sourceZ` (HF damping + LF corner), `loss` (`b` in Eq 17, 0 = lossless line iso), `air` (leakage HF Q). Paper-validated against **two** guitar-amp OTs (Fender NSC041318 + Hammond T1750V); preset roadmap below — `tubeOT` and `hammondOT` paper-validated; everything else flagged "v0 extrapolation" until bench measurements come in. Backs both Pultec OT stage (#140) and standalone "iron" character. **Preset roadmap (6 famous-transformer voicings, 2026-04-27 from Kazrog True Iron audit):** (1) `marinair` — Marinair LO1166/A (THE Neve mic-pre transformer; signature British-console warmth + mid-bump character); (2) `utc108x` — UTC 108 X (American broadcast/studio line transformer; flatter than Marinair, more linear top); (3) `malotki` — Malotki E4M-4001B (German continental studio transformer; Telefunken-era, slightly tighter LF); (4) `we111c` — Western Electric 111C (telephone-era; vintage radio/tape voicing, narrower bandwidth); (5) `haufeV178` — Haufe V178 (German broadcast/DDR-era; classical-recording-grade, very low THD); (6) `utcO12` — UTC O-12 (output transformer; line-stage / OT character, more LF saturation than 108X). Plus the existing `tubeOT` (Fender NSC041318), `hammondOT` (Hammond T1750V), `line` (clean iso, default), `micPre` (extrapolation). Each preset = pre-tuned `(coreSize, sourceZ, loss, air)` quad. Bench measurements needed for marinair/utc/malotki/we/haufe/utcO12 before flipping any to "paper-validated" — currently all v0 extrapolations except tubeOT and hammondOT. |
...
| 184 | bootstrapLF | ⬜ | Character/Tone | **Rod Elliott (Sound-au.com), "Bootstrapping" — sound-au.com/articles/bootstrap.htm** + **everycircuit.com/circuit/6694255225733120 (interactive A/B sim of both topology variants)**. Analog circuit technique where AC voltage across a resistor is held constant via capacitive feedback from a unity-gain buffer. Two variants both expressible in one op: **(1) input-impedance boost** (emitter-follower bootstrap, raises input Z — primarily affects LF response near coupling-cap corner) and **(2) gain enhancement** (split collector load with center-tap bootstrap cap = constant-current AC behavior, ~2× gain + ~3× THD reduction in valves; 12AX7 example 0.48% → 0.16%). Both manifest as a tunable resonant HPF (corner 1–50 Hz, Q variable up to ~20 dB peak — typical 10 nF + 10 µF caps land 20 dB peak at ~6 Hz) coupled with an optional `linearity` parameter that injects a distortion-reduction coefficient when chained into a saturating cell (`tubeSim` / `triodeStage` / `cathodeBiasShift`). Tier-A character op — queued 2026-04-27 by Stav. |
| 185 | samplerNonlinearity | ⬜ | Character / ADC | **Razavi, "The Design of a Bootstrapped Sampling Circuit," IEEE Solid-State Circuits Magazine, Winter 2021 (DOI 10.1109/MSSC.2020.3036143). PDF at `docs/primary_sources/razavi/BR_SSCM_1_2021.pdf` (also seas.ucla.edu/brweb/papers/Journals/BR_SSCM_1_2021.pdf).** Models the *non-bootstrapped* sampler's distortion as a character effect (body-effect modulation: V_GS varies with V_in → R_on varies → 3rd-harmonic distortion at the track-mode output). Razavi's worked example: 28 nm CMOS sampler at 5 GHz / V_in 0.25–0.75 V → measurable 3H at ~-83 dB after bootstrap, and significantly worse without. DSP model: track-and-hold front end + on-resistance modulation as f(V_in) → odd-harmonic generator (mainly 3H, weaker 5H/7H). Two sub-modes: `bootstrapped` (clean, gentle character — useful as a "polish" stage at the end of a chain) and `non_bootstrapped` (lofi-A/D color, body-effect 3H at -60 dB to -40 dB depending on swing). Pairs naturally with `srcResampler` to build "vintage A/D" recipes (Lavry-style vs cheap-converter A/B). Tier-B character op — queued 2026-04-27 by Stav. |
| 186 | crossoverDistortion | ⬜ | Character / Dynamics | **Cordell, *Designing Audio Power Amplifiers* (McGraw-Hill 2010), Ch 5 §5.4 ("Crossover Distortion").** Models the canonical class-AB crossover glitch: when both push-pull output devices momentarily stop conducting near the zero crossing (under-biased) or both conduct briefly (over-biased), the transfer curve has a small "kink" at zero. Audible as a high-frequency buzz/grit on quiet passages, especially on sustained notes where the signal lingers near zero. DSP model: dead-zone / overlap nonlinearity around `x ≈ 0`, parameterized by `bias_amount` (negative = under-biased gap, positive = over-biased shelf, 0 = ideal class-A). Pairs naturally with `tubeSim` / `pushPullPower` (#155) to build vintage solid-state amp recipes. Distinct from `hardClip` (clipping at the rails) and `saturate` (smooth tanh) — crossoverDistortion specifically adds artifacts at low signal levels, the opposite of soft-saturator behavior. Tier-A character op — queued 2026-04-27 by Stav from Cordell book audit. |
| 187 | domPoleAmp | ⬜ | Character / Tone | **Cordell, *Designing Audio Power Amplifiers* (McGraw-Hill 2010), Ch 6 (NFB) + Ch 7 (Compensation).** Models a Miller-compensated dominant-pole amplifier with negative feedback: linear in the small-signal regime (loop gain dominates), but with **slew-rate-limited large-signal behavior** on transients — fast attacks get rounded off as the amp's output stage can't keep up with the input slope. Cordell ch.6 §6.2 covers slew-rate origin (compensation cap charging current); ch.7 covers Miller / two-pole-compensation / nested topologies. DSP model: linear gain stage + per-sample slew-rate limiter + dominant-pole IIR (single high-Z pole around 100 kHz typical) + small THD floor as a tunable param. Effect: drum transients lose top-end "snap" subtly, sustained tones unaffected. Vintage hi-fi amp character — distinct from saturate/clip because it's TIME-DOMAIN limiting, not amplitude-domain. Could share infrastructure with #92 slew op + adds the dominant-pole shaping. Tier-B character op — queued 2026-04-27 by Stav from Cordell book audit. **Borderline:** could fold into existing `slew` if we expose `dominantPoleHz` as a param there, instead of new slot. Decide at build time. |
| 188 | riaaEQ | ⬜ | EQ / Vintage | **Mullard 1963 *Circuits for Audio Amplifiers* (TP413), magnetic gramophone preamp section** + **IEC 60098 / RIAA standard curve.** RIAA playback equalization curve for vinyl phonograph signals: poles at 50.05 Hz and 2122 Hz, zero at 500.5 Hz → +20 dB LF boost / -20 dB HF cut to undo the inverse curve baked into vinyl record cutting. DSP model: 3-section IIR (or 2-pole/1-zero biquad chain) with the standard time constants τ_1=3180μs, τ_2=318μs, τ_3=75μs. Optional `loading_R` and `loading_C` params model cartridge-load interaction (shifts HF response). Specialty op for phono / vintage-vinyl recipes. Tier-A — queued 2026-04-27 by Stav from Mullard book audit. |
| 189 | baxandallTone | ⬜ | EQ / Tone | **Mullard 1963 *Circuits for Audio Amplifiers* (TP413), tone-control section** + **Baxandall, "Negative-Feedback Tone Control," Wireless World, 1952.** The classic British active bass/treble tone stack: shelving low + shelving high with a flat midband, all built around a single op-amp / triode with negative feedback. Distinct from American "Fender" tone stack (passive, mid-scoop) — Baxandall is feedback-based with flat-when-centered behavior. DSP model: low-shelf biquad + high-shelf biquad with linked center param. Tier-A — queued 2026-04-27 by Stav from Mullard book audit. |
| 190 | ultraLinearScreen | ⬜ | Character / Tube | **Mullard 1963 *Circuits for Audio Amplifiers* (TP413), Mullard 5-20 ultra-linear power amp section.** Models the ultra-linear (UL) screen-grid tap on a push-pull tube power amp's output transformer primary — sits between pure pentode (high efficiency, more 3H) and pure triode (low distortion, lower power) operation. The UL ratio (typically 43% screen tap) gives a unique compromise distortion signature. DSP model: parameter on `#155 pushPullPower` (`pentode_ratio` 0.0–1.0) rather than a standalone op. **Borderline — likely folds into #155 at build time.** Reserved for the case where UL behavior is complex enough to warrant its own slot. Tier-B — queued 2026-04-27. |
| 191 | tubeRectifierSag | ⬜ | Character / PSU | **Mullard 1963 *Circuits for Audio Amplifiers* (TP413), GZ34 rectifier specifications** + **Cordell Ch 12 (cross-ref).** Tube-rectifier (GZ34/5AR4 etc.) version of #154 psuRectifierSag. Distinct from solid-state rectifier sag because tube rectifiers have HIGHER internal resistance (gentler sag onset, more compression-like character on heavy program), and respond to overdrive with classic "tube amp breathing." DSP model: peak/RMS-following sag amount with frequency-dependent compression (Mullard GZ34 datasheet curves). **Borderline — could be a `rectifier_type: 'tube'` mode on #154 instead of separate slot.** Tier-B — queued 2026-04-27. |
| 192 | williamsonAmp | ⬜ | Recipe / Character | **D.T.N. Williamson, "Design of a High-Quality Amplifier," Wireless World, April–May 1947** + **Mullard 1963 *Circuits for Audio Amplifiers* (TP413) — references Williamson topology throughout.** The 1947 Williamson topology is the foundational British hi-fi tube power amp: long-tail-pair phase splitter + push-pull triode-connected output stage + global negative feedback. Influenced every British amp design afterward (Quad, Leak, Radford). **Likely a RECIPE rather than an op slot** — composed from existing ops: `triodeStage` + `phaseInverter` (LTP) + `pushPullPower` + global feedback wiring. Marked as op slot for now in case feedback-loop topology requires a dedicated wrapper. Tier-A historical — queued 2026-04-27. |
| 193 | outputStageProtection | ⬜ | Character / Dynamics | **Cordell *Designing Audio Power Amplifiers* (McGraw-Hill 2010), Ch 13 (Protection Circuits).** Models SOA (safe-operating-area) limiting and V-I current foldback in solid-state output stages. When the amp gets driven into a low-impedance load too hard, protection kicks in: gain falls off non-linearly, distortion spikes, sometimes triggers shutoff. Audible as "amp dying" character — clipping that gets WORSE rather than just clipping at a hard ceiling. Niche; relevant for "vintage amp under abuse" recipes (e.g. emulating a small SS amp pushed past its limits). DSP model: hysteresis-tracked I_out vs V_out crossing → gain reduction + harmonic chaos. Tier-C — queued 2026-04-27 from Cordell book audit. **Defensible to skip — small audience.** |
| 194 | thermalDistortion | ⬜ | Character / Time-domain | **Cordell *Designing Audio Power Amplifiers* (McGraw-Hill 2010), Ch 5 (Distortion Mechanisms — Thermal).** Models slow-drift gain modulation in BJT output stages caused by die-temperature change under varying program. On a sustained note, the BJT junction warms → β changes → gain drifts subtly over hundreds of milliseconds. Effect: very slow envelope-rate amplitude breathing, separate from the audio signal itself. Subtle — only audible on long sustained tones at high SPL. DSP model: sliding-RMS-driven gain modulator with τ ~500 ms. Tier-C — queued 2026-04-27 from Cordell book audit. **Borderline subtle — sign-off may be hard to discriminate.** |
| 195 | hawksfordVAS | ⬜ | Character / Transparent | **Cordell *Designing Audio Power Amplifiers* (McGraw-Hill 2010), Ch 8 (VAS — Hawksford topology)** + **Malcolm Hawksford, "The Essex Echo," Hi-Fi News & Record Review (1980s articles).** Models a Hawksford-style nested-feedback voltage-amplifier stage with very low THD. Target: "hi-fi clean" — intentionally NO character, used as a reference / null point. DSP role: linear gain with measured THD floor below the 0.001% range. **Borderline — transparent ops are functionally equivalent to a clean `gain`. The slot exists in case we need a "verified-transparent" op for null-test reference recipes.** Tier-C — queued 2026-04-27 from Cordell book audit. |
| 196 | virtualEarthBuss | ⬜ | Routing | **EMI TG12345 Mk2 Service Handbook §2.4.2 + §3.5.8 (Amp B + Amp C topology).** PDF at `docs/primary_sources/abbey_road/EMI_TG12345_Mk2_Service_Handbook.pdf`. Voltage-to-current converter (Amp B, ~20 kΩ output Z) per source channel feeding a virtual-earth summing junction (Amp C, ~20 Ω input Z). Distinct from #6 mix (which is parallel weighted-sum of audio buffers). Each added channel only changes existing-channel gain by ~0.01 dB; 20-channel sum = 0.2 dB drift max — negligible. **The defining Abbey Road buss-summing character: clean, low-crosstalk, high-headroom blooming when many channels add up.** DSP model: per-channel V→I gain stage (gain = 1/Z_out) into a single summing accumulator with the ~20 Ω virtual-earth load modeled as cumulative-gain-shift coefficient. Tier-S — queued 2026-04-28 from EMI TG12345 ingestion. |
| 197 | abbeyPresenceEQ | ⬜ | Tone / EQ | **EMI TG12345 Mk2 Service Handbook §3.5.4 + §12.1 + TG Type X Presence Control schematic** (`docs/primary_sources/abbey_road/germanium/TG Type X Presence Control.pdf`, drawn by CR 21/03/21). Mid-band peak EQ with 8 selectable centre frequencies × ±10 dB in 11 × 2 dB steps. Frequencies: **0.5 / 0.8 / 1.2 / 1.8 / 2.8 / 4.2 / 6.5 / 10 kHz**. **TOPOLOGY (locked from schematic):** passive LC-tank-bank + active makeup. Two large air-core inductors L2 = 665 mH and L3 = 184 mH with multiple winding taps + cap-bank selection (100 nF / 18 nF+68 nF / 47 nF / 8.2 nF / 18 nF / 8.2 nF / 4.7 nF / direct), 1 MΩ damping resistor per tank, Q ≈ 6 typical. Active stage = BC109+BC189 complementary feedback amp on Board B51/3 (R5=20k + R6=300R + C5=150p inner FB loop). 12-position Rs1...Rs12 stud-switch ladder picks boost/cut tap. Distinct from #144 inductorEQ (Pultec) — TG presence is single-band-at-a-time (rotary), Pultec is two-band-simultaneous (concentric). See `memory/emi_tg12345_console.md §10c.2`. Tier-S — queued 2026-04-28, schematic-locked 2026-04-28. |
| 198 | abbey500HzBass | ⬜ | Tone / EQ | **EMI TG12345 Mk2 Service Handbook §3.5.5 + Drawing 375 + TG Type Z Bass schematic** (`docs/primary_sources/abbey_road/germanium/TG Type Z Bass.pdf`, drawn by CR 21/03/21). "Bass" control whose **half-lift frequency is at 500 Hz** (not a low shelf — low-mid lift/scoop). ±10 dB range in 11 × 2 dB steps. **TOPOLOGY (locked from schematic):** single LR network with **L1 = 1.93 H** massive air-core inductor + R2 = 750 Ω series resistor → electrical corner at 62 Hz (ω = R/L), but the **ear-rated half-lift sits at 500 Hz** because the 6 dB/oct asymptote is several octaves above the electrical pole. Same B51/3 active stage as #197 abbeyPresenceEQ (BC109+BC189 complementary), same 12-step Rs1...Rs12 ladder. **Critical character note:** boosting "bass" adds energy in the lower midrange UP TO 500 Hz — the warm-but-not-sub Abbey Road tonal fingerprint. See `memory/emi_tg12345_console.md §10c.3`. Tier-S — queued 2026-04-28, schematic-locked 2026-04-28. |
| 199 | be740BassCut | ⬜ | Tone / EQ | **EMI TG12345 Mk2 Service Handbook BE740 schematic.** 6-position octave-spaced 12 dB/oct highpass: **FLAT / 75 / 150 / 300 / 600 / 1200 Hz**. Discrete cap-bank rotary network: R = 1 MΩ throughout, C bank values 0.22 / 0.10 / 0.047 / 0.022 / 0.01 μF (HP section) + 0.33 / 0.15 / 0.068 / 0.033 / 0.015 μF (LP shaping) per channel. Each frequency step doubles — clean octave grid for surgical low-end cleanup pre-track. Distinct from `dcBlock` (always-on subsonic) and #2 filter (continuous biquad). DSP model: stepped 2nd-order Butterworth HPF with 5+1 fixed cutoffs. Tier-S — queued 2026-04-28 from EMI TG12345 ingestion. |
| 200 | abbeyChannelBPF | ⬜ | Tone / EQ | **EMI TG12345 Mk2 Service Handbook §2.4.7 + §12.7 (Band Pass Filter).** Channel-resident always-on bandpass: 12 dB/oct subsonic <30 Hz + 20 kHz 1st-order LPF top, with **AOT (adjust-on-test) trim resistors** at both corners to compensate for cumulative coupling-cap roll-off and HF gain variations. Function: keeps the channel response substantially flat 30 Hz–20 kHz independent of how many cassettes the signal traverses. DSP model: 2-pole HPF @ 30 Hz + 1-pole LPF @ 20 kHz with `bass_aot` and `treble_aot` ±2 dB trim params. Tier-S — queued 2026-04-28 from EMI TG12345 ingestion. |
| 201 | tg12345CompLim | ⬜ | Compressor | **EMI TG12345 Mk2 Service Handbook §3.5.2 + §12.22 + TG Limiter "Zener Limiter 1970" schematic** (`docs/primary_sources/abbey_road/germanium/TG Limiter.pdf`, drawn by CR). **TOPOLOGY (locked from schematic):** **back-to-back zener-clip-in-feedback** — a category not represented elsewhere in the catalog. **D1, D2 = HS2851 ~5.6 V zener pair** in shunt across the audio amp's feedback path. Below threshold: linear gain. Above ~5.6 V signal swing across the diode pair: zeners conduct → shunt feedback current → effective gain falls. **Soft knee is built into the V-I curve of the zener junction itself** — no explicit knee-radius math needed. Detector: full-wave **BAY38 back-to-back rectifier** + peak-storage cap → control voltage. 3-position key (Out/Compress/Limit) selects feedback ratio: SL2 = compress (softer knee), SL1 = limit (harder knee). **6 fixed recovery times** (0.1/0.25/0.5/1/2/5 s) via stud switch into C11=20μ + C12=1μ smoothing caps. 10 kΩ log carbon Hold pot. Gang-detect: max(L_cv, R_cv) diode-OR. GR meter taps directly across the zener pair. Distinct from #142 blackmerVCA (log-exp OTA), #145 varMuTube (remote-cutoff pentode), #147 fetVVR (variable-resistance JFET), #179 diodeBridgeGR (matched-diode bridge). **The defining post-1968 Abbey Road compression character** — heard on every late-Beatles record, *Dark Side of the Moon*, etc. See `memory/emi_tg12345_console.md §10c.1`. Tier-S — queued 2026-04-28, schematic-locked 2026-04-28. |
| 202 | tg12345MicAmp | ⬜ | Character | **EMI TG12345 Mk2 Service Handbook §3.4.2 + §12.27 + BE232 (Amp D) + David Reess June 1968 sketchbook B112 prototype** (`docs/primary_sources/abbey_road/germanium/1968_06-David-Reess-sketchbook-notes-for-B112_B183AV-circuits-from-JT-1200px.jpg`). Discrete 3-stage common-emitter mic preamp. **MEASURED DATA (Reess prototype B112, June 1968):** EIN = **−120 dBm with 5 kΩ source = 1 dB NF** (Johnson floor for 4K8 = −116 dBm; amp delivers 4 dB headroom below source thermal noise). Loop gain = 66 dB. Output: +16 dBm into 5 kΩ before clipping. **THD scales with gain at 10 kHz: 0.012% (18 dB) → 0.4% (58 dB) — 40× growth across 40 dB.** LF/MF THD ≈ 0.10% constant above 23 dB. 9-position prototype gain table: 18/23/28/33/38/43/48/53/58 dB (R = ∞/330/120/56/27/15/8.2/3.9/1.8 Ω feedback selector); production simplified to 12×5 dB coarse + Amp D ±5 dB fine. Mixed BC109/BC107/BC108 in prototype, BC109×3 in production. Distinct from #146 discreteClassAStage (Neve 1073/API 312, +24 dBV rail) and #148 triodeStage (tube). See `memory/emi_tg12345_console.md §10c.4`. Tier-S — queued 2026-04-28, prototype-data-locked 2026-04-28. |
| 203 | germaniumBuffer | ⬜ | Character | **4 independent schematics: AE207 (Amp E) + AE209/AE210 (Amp G) + TG Type R (Output stage, §10d.2) + Robinson Buffer 1970 (`docs/primary_sources/abbey_road/germanium/Robinson & Levesley/Robinson Buffer.pdf`).** Discrete two-stage buffer: silicon-NPN input (BC109) cascaded into a **germanium-PNP output stage** — **ACY21** in EMI / **2N4062** in Robinson / **AC176** in Chandler-Neve clone (B104A topology). Defining feature: Ge V_BE knee ≈ 0.2–0.3 V (vs Si 0.6–0.7 V) → **softer asymmetric distortion onset** + **warmer 2H/3H balance** at the clipping edge. **Period-wide UK convention 1968–1972** — used by EMI (Amp E/G/R), Neve (B104A), Robinson (Helios-era Buffer/Out). The TG12345 routes EVERY post-fader signal through this topology — hidden cumulative character signature explaining the "smooth/rounded" Abbey Road / British-70s colour. DSP model: clean linear gain + tunable Ge-knee softclip with frequency-independent 2H bias. Sub-presets: `abbeyMakeup` (Amp E +5 dB), `abbeyUnityBuffer` (Amp G 0 dB), `abbeyOutput` (Amp R, with VU tap), `robinson1970` (Robinson Buffer 0/+10/+20 dB), `neveB104A` (Chandler/Neve clone path). Tier-S — queued 2026-04-28, schematic-locked across 4 independent sources 2026-04-28. **The most important "hidden" finding from the TG12345 + Germanium-folder ingestion.** |
| 204 | neveB104A | ⬜ | Character | **Chandler Germanium Pre schematic** (`docs/primary_sources/abbey_road/germanium/Chandler Germanium Pre/Germanium Pre.pdf`), explicitly labeled "COPY OF NEVE B104A". Mic preamp module from Neve's late-60s consoles (1064 / 1066 / 1073 predecessors). 3-stage **mixed silicon-NPN + germanium-NPN driver + silicon-power-BJT output**: Q1=BC184C silicon input → Q2=AC176 germanium driver → Q3=MJ2955 silicon power BJT output. **Distinct from #146 discreteClassAStage** (which models the later Neve 1073 BC184/2N3055 all-silicon era) and **distinct from #202 tg12345MicAmp** (BC109 × 3 all-silicon, ±20 V rails). Power: +28 V single-rail. Master gain via 4K7 feedback pot. Modern Chandler Germanium Pre product = direct descendant of this topology with current-production transistors. Tier-A — queued 2026-04-28 from Germanium-folder ingestion. |
| 205 | robinsonTone | ⬜ | Tone / EQ | **Robinson 1970 Tone schematic** (`docs/primary_sources/abbey_road/germanium/Robinson & Levesley/Robinson Tone.pdf`, drawn by CR 22/03/21). 3-band active tone control with **inductor-tuned treble** — a distinct British-70s topology from Baxandall's all-RC approach. **Bass:** VR1 = 500 K LIN with R6/R7 = 4K7 + C6/C7 = 100 nF RC. **Mid:** VR2 = 25 K LIN with R8 = 22 K + C8 = 1.5 nF RC. **Treble:** VR3 = 5 K LIN with **L1 = 86 mH air-core inductor + C9 = 47 nF LC tank → resonance ~2.5 kHz** (gives treble a Q-peak character distinct from RC-only Baxandall). 3-stage BC109 active makeup with feedback-around-tone-stack topology, +24 V single-rail. Used in Helios-era and Robinson-built consoles. Tier-A — queued 2026-04-28. |
| 206 | porterTone | ⬜ | Tone / EQ | **Porter (B.E.), "Tone control" Wireless World January 1984, p73** (`docs/primary_sources/abbey_road/germanium/Wireless World/Wireless-World-1984-01 p77 Porter.pdf`). NE5534 op-amp non-Baxandall tone control. Claimed advantages: (1) no interaction (pots terminated by virtual-earth), (2) adjustable LF + HF turnover points, (3) true shelving with exact mirror images for lift/cut, (4) non-inverting → easy bypass. Single-pole filters by default; **single-pole filters can be replaced by state-variable filters for full parametric control**. ±10 dB lift/cut at R_A/R_B = 4K7. Component values: 10 K + 22 pF feedback IC1/IC2, 6K65 + 1n2 + 50 K anti-log LF select, 20 K lin HF select, 150 nF LP, 22 μF/16 V output. Period-correct British-80s op-amp-era reference. Tier-A — queued 2026-04-28. |
...
## Cross-references — 2026-04-28 deep-research sweep (additive supplements)

**Source.** Two parallel deep-research passes (Gemini 2.5 Pro + Perplexity Pro) on `memory/perplexity_research_brief.md`. Full URLs, contradictions, and 30 archetype recipes captured in `memory/research_findings_2026_04_28.md` and `memory/recipe_library.md`.

**Discipline.** ADDITIVE-ONLY. None of the entries below modify the locked Tier-S rows in any preceding section. They supplement those rows with **open-access PDF URLs** and **newly identified secondary primaries**. Where a row had a missing or open-question citation, the new finding is marked `(NEW)`. Where the row already had a citation but the PDF location was previously informal, the URL is marked `(URL)`.

### Newly queued slot rows (#184–#195)

| catalog # | opId | supplementary primary added 2026-04-28 |
|---|---|---|
| #184 | bootstrapLF | (NEW) Self, *Small Signal Audio Design* 3e (Routledge 2020) Ch.2; + Self, "Distortion in Power Amplifiers Part III: The Voltage-Amplifier Stage," *Electronics World* 1993 — open mirrors at burosch.de and eetimes.com. Cordell 2010 Ch.2 cross-ref for THD profile. |
| #188 | riaaEQ | (NEW) Lipshitz, "On RIAA Equalization Networks," *JAES* 27(6):458–481, June 1979 — **open PDF** at keith-snook.info/riaa-stuff/Lipshitz%20AES.pdf. Tier-S canonical mathematical reference; full transfer equations + 4 network topologies. |
| #189 | baxandallTone | (URL) Baxandall, "Negative-Feedback Tone Control," *Wireless World* 58(10):402–405, October 1952 — open PDFs at hb9aik.ch/Audio/Baxandall_Notes_V2.pdf and thermionic.info/baxandall/Baxandall_NegativeFeedbackTone.pdf. |
| #192 | williamsonAmp | (NEW) Williamson, "Design For A High-Quality Amplifier," *Wireless World* April + May 1947 — open PDFs at worldradiohistory.com/UK/Wireless-World/40s/Wireless-World-1947-04.pdf and -1947-05.pdf; compiled booklet at introni.it/pdf/The Williamson Amplifier.pdf. Triode-connected KT66 PP, 20 dB NFB inc. OT, GZ32/34 rectifier. |
| #195 | hawksfordVAS | (NEW) Hawksford, "Distortion Correction in Audio Power Amplifiers," *JAES* 29(1/2):27–30, Jan/Feb 1981 (AES e-lib elib:3935, paywalled); + "The Essex Echo" *HFN/RR* series, open compiled PDF at tmr-audio.de/pdf/Hawksford_Essex.pdf and linearaudio.net. **(UPDATE 2026-04-28 PM)** Hawksford 1981 part-2 ("Distortion Correction Circuits for Audio Amplifiers" *JAES* 29(7/8):503–510) **NOW OPEN** at researchgate.net/profile/Malcolm-Hawksford-2/publication/269093795 — closes the AES paywall gap for one of the two locked papers. ChatGPT deep-research third-validation pass confirmed Hawksford as the canonical primary, agreeing with Perplexity + Gemini. |

### Existing-row supplements (open-PDF URLs + newly identified secondary primaries)

| catalog # | opId | supplementary primary / URL added 2026-04-28 |
|---|---|---|
| #143 | bjtSingleStage | (NEW supp.) Self, *Small Signal Audio Design* 3e Ch.3–4 — preamp-specific BJT VAS analysis with measured THD tables. Cordell 2010 stays primary (locked). |
| #144 | inductorEQ | (URL) API 550A schematic — open PDF at crasno.ca/articles/doc/API_550A_SCH.pdf and barryrudolph.com/recall/manuals/api550a.pdf. Includes discrete op-amp (AP2520) + frequency-selection switching. |
| #146 | discreteClassAStage | (NEW supp.) Pass, "Burning Amplifier #1" + "Leaving Class A" + "The A75 Power Amplifier Part 1" — open PDFs at firstwatt.com/wp-content/uploads/. Bench-validated schematics with measured distortion spectra and bias currents. |
| #148 | triodeStage | (URL) RCA Receiving Tube Manual RC-30 (1975) — open PDFs at archive.org/details/RCA_RC-30_1975 and tubebooks.org/tubedata/rc30.pdf. (URL) Pakarinen-Yeh CMJ 2009 — open PDF at effectrode.com/wp-content/uploads/2018/08/a_review_of_digital_techniques_for_modeling_guitar_amplifiers.pdf. (NEW supp.) Macak, *Real-Time Digital Simulation of Guitar Amplifiers as Audio Effects*, PhD thesis Brno 2012 — DSpace partial at dspace.vutbr.cz. |
| #149 | otaLadder | (NEW) Steiner, "Voltage-Tunable Active Filter Features Low, High and Bandpass Modes," *Electronic Design* 22(25):96, December 1974 — **open PDF** at yusynth.net/archives/ElectronicDesign/N-Steiner-VCF-1974.pdf. Original Steiner-Parker filter publication; full schematic + component values. |
| #150 | granularPitchShifter | (URL) De Götzen-Bernardini-Arfib DAFx-2000 — open PDF at cs.princeton.edu/courses/archive/spr09/cos325/Bernardini.pdf. Includes full reference MATLAB code under GPL. |
| #151 | bridgedTNetwork | (NEW supp.) Zverev, *Handbook of Filter Synthesis* (Wiley 1967) Ch.6 — bridged-T + twin-T transfer derivations with measured responses. Williams-Taylor *Electronic Filter Design Handbook* 4e (McGraw-Hill 2006) for practical component tables. No open-access PDFs found. |
| #152 | vactrolLPG | (URL) Parker-D'Angelo, "A Digital Model of the Buchla Lowpass-Gate," DAFx-2013 — open PDF at dafx.de/paper-archive/2013/papers/44.dafx2013_submission_56.pdf. (NEW supp.) DAFx-2023 power-balanced vactrol dynamics paper at dafx.de/paper-archive/2023/DAFx23_paper_50.pdf. (NEW) VTL5C3/2 datasheet at micros.com.pl/mediaserver/info-oovtl5c32.pdf — canonical hardware spec. (URL) Buchla 259 user guide 1981 at modularsynthesis.com/roman/buchla259/. |
| #155 | pushPullPower | (NEW supp.) "Simulation of a Vacuum-Tube Push-Pull Guitar Power Amplifier," DAFx-2011 — open PDF at dafx.de/paper-archive/2011/Papers/05_e.pdf. Includes OT + loudspeaker interaction. |
| #157 | cathodeBiasShift | (NEW supp.) Yeh-Abel-Chaudhary-Smith, "Automated Physical Modeling of Nonlinear Audio Circuits for Real-Time Audio Effects," IEEE *TASLP* Part I (2008) + Part II (2012). Authoritative automated nonlinear-circuit modeling paper. |
| #158 | bridgedTeeEQ | (URL) Pultec MEQ-5 schematic + manual — open PDFs at funkwerkes.com/web/wp-content/techdocs/MixedProAudio/Pultec-MEQ-5-Alt-EQ-Filter-Schematic.pdf and Pultec-MEQ5-Manual.pdf; gyraf.dk/wp-content/uploads/2018/05/Do-A-Pultec.pdf. Inductor values 420/520/477 mH. |
| #159 | dopplerRotor | (NEW supp.) Smith-Serafin-Abel-Berners, "Doppler Simulation and the Leslie," DAFx-2002 — open PDF at dafx.de/papers/DAFX02_Smith_Serafin_Abel_Berners_doppler_leslie.pdf. Canonical signal-processing model. (NEW supp.) Pakarinen-Karjalainen "Computationally Efficient Hammond Organ Synthesis" DAFx-2011 at dafx.de/paper-archive/2011/Papers/49_e.pdf. |
| #162 | ringMod | (NEW supp.) Hoffmann-Burchardi, "Digital Simulation of the Diode Ring Modulator for Musical Applications," DAFx-2008 — open PDF at dafx.de/paper-archive/2008/papers/dafx08_29.pdf. (NEW supp.) DAFx-09 BJT ring-mod paper at dafx.de/paper-archive/2009/papers/paper_24.pdf. |
| #163 | hardSync | (NEW supp.) Lazzarini-Timoney, "Virtual Analog Oscillator Hard Synchronisation," DAFx-2012 — open PDF at dafx12.york.ac.uk/papers/dafx12_submission_37.pdf. (URL) Välimäki-Huovilainen, "Antialiasing Oscillators in Subtractive Synthesis," IEEE SPM 24(2):116–125, March 2007 — already locked supp.; URL via academia.edu/97506982. |
| #174 | phaseDistortion | (NEW) US Patent **4,658,691** (Hatanaka/Kashio, Casio) — patents.google.com/patent/US4658691A/en. Original phase-distortion address-modification hardware. (NEW supp.) Lazzarini et al. "Adaptive Phase Distortion Synthesis" — open PDF at mural.maynoothuniversity.ie/id/eprint/2335/1/VL_Adaptive_Phase_paper_12.pdf. |
| #175 | wowFlutter | (NEW supp.) Zavalishin-Parker, "Efficient Emulation of Tape-Like Delay Modulation Behavior," DAFx-2018. (NEW supp.) "Real-Time Physical Modelling for Analog Tape Machines," DAFx-2019 — open PDF at dafx.de/paper-archive/2019/DAFx2019_paper_3.pdf. Bertram 1994 + IEC 386:1971 stay locked primaries. |

### Open questions logged (no Tier-S found this sweep)

| catalog # | opId | open question |
|---|---|---|
| #152 | vactrolLPG | "Felt 2010 vactrol thermal model" paper not located; closest match is the DAFx-2023 power-balanced paper. Flag for human verification. |
| #153 | gateStateMachine | No new Tier-S beyond Zölzer DAFX Ch.4 + Giannoulis-Massberg-Reiss 2012 (already locked). Drugan-Reiss 2017 confirmed fabricated in earlier sweep. |
| #162 | ringMod | "Bogdanowicz-Belkin Bode article" not located under those author names. DAFx-2008 + DAFx-2009 above are the strongest confirmed primaries. |
| #175 | wowFlutter | "Wright DAFX 2017" not confirmed at the exact citation; closest matches are Zavalishin-Parker DAFx-2018 + DAFx-2019. Flag for human verification. |
| #178 | differentialEnvelope | No standalone academic paper at Tier-S level for transient-shaper attack/sustain detector. Cordell Ch.5 + Zölzer DAFX Ch.4 + SPL service docs (locked) remain the corpus. Logged as open. |
| #184 | bootstrapLF | (Original SSAD PDF still not located.) **(UPDATE 2026-04-28 PM)** Self *Audio Power Amplifier Design Handbook* 4th ed. (Newnes 2006) **NOW OPEN** at nick.desmith.net/Data/Books and Manuals/Self - Audio Power Amp Design Handbook 4th Edn.pdf — VAS bootstrapping discussion at pp. 97–100 covers the topology and one-axis distortion impact. Closes "no open Self PDF" gap; the requested 3-axis THD-vs-frequency-vs-source-impedance sweep is still unresolved (parked). |

### Historical contradictions flagged (human review only — not for ingestion)

Five disputes identified across the research that **do not contradict any locked source**, logged here so future readers know the topic is contested:

1. **RIAA derivation credit** — Wright literature claims Lipshitz omitted t6 HF corner correction; Lipshitz 1979 expressly includes it.
2. **Baxandall tone-control authorship** — Baxandall (1952) universally credited, but Burwen filed identical active-topology patent in 1950; passive precursor by E.J. James 1949 / Volkoff ~1938.
3. **SSL G-Series IN-lamp logic** — manufacturer says series, field techs report parallel.
4. **Vari-Mu component designations** — Altec 436C vs. Manley Variable Mu schematic numbering disagree despite shared compression mechanics.
5. **"Warm-up time"** — pervasive myth contradicted by Self's bench measurements (DC stable within seconds).

These don't gate any ship decision; logged for context.
