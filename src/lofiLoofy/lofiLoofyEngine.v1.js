...
export function createLofiLoofyEngineV1(ctx) {
  ...
  return {
    ...
    // ── QC CAPABILITY SCHEMA (canonical — memory/qc_family_map.md §2) ────
    // Declares what this plugin IS so the QC rack selects the right tests
    // and gates the right rules. Must reflect the engine as-implemented,
    // not the aspirational design. Changing a DSP stage ⇒ update these.
    capabilities: {
      categories: ['Lo-fi', 'Modulation'],
      subcategories: ['bitcrusher', 'saturator-soft', 'tape-emulation',
                      'compressor-parallel', 'reverb-send', 'stereo-widener'],
      modes: ['default'],
      ...
      hasFeedback:      false,
      hasFreeze:        false,
      hasLFO:           true,   // Dream / drift / flutter modulators
      hasStereoWidth:   true,   // widener stage
      hasMultiband:     false,
      hasLookahead:     true,   // DynamicsCompressor ≈ 6 ms internal lookahead
      hasTruePeak:      false,
      ...
      // Numeric / structural
      // nonlinearStages counts explicit waveshaper-type stages in the wet
      // chain: satShape (2x), ageCrush (2x), bitsShaper (none),
      // crushShape (none). Compressors are counted under Dynamics, not here.
      nonlinearStages:  4,
      osThresholds:     null,   // no user-facing OS switch; per-stage static
      // NOTE: real latency is 36 ms (dryCompensate 30 ms + comp lookahead 6 ms,
      // see getLatency()). latencySamples is expressed at the QC rack's
      // running sample rate; the engine declares the source-of-truth in
      // seconds via getLatency(). Leaving 0 here suppresses the
      // latency_report preset until the capability schema gains an SR-aware
      // field — tracked as a follow-up. The phase-relationship is still
      // checked via mix_null / mix_null_series.
      latencySamples:   0,
      crossoverPhase:   null,

      // Clipping: Lofi Loofy is tanh-based saturation throughout — no
      // deliberate hard-clip stage. Bit-crushing is quantization, not
      // clipping, and is declared via nonlinearStages.
      isClipper:        false,

      // Coloration gating:
      //   dryLegHasColoration = false
      // The topology is `input → inGain → dryCompensate(36 ms delay) →
      // dryGain → preOut`. The dry leg is a pure delay line with NO tonal
      // shaping — fundamentally different from ManChild's Fairchild
      // emulation where the tubes stay in-circuit. Consequently the
      // standard mix_null contract DOES apply: at Mix=0 the output should
      // null to within −80 dB against a delay-compensated input.
      //
      // WARNING (pre-existing, pending verification): this engine mixes
      // dry/wet via EXTERNAL GainNodes (dryGain / wetGain) with a
      // DelayNode (dryCompensate, 36 ms) attempting to match the wet
      // chain's group delay. dry_wet_mix_rule.md §1 declares this pattern
      // forbidden — oversampled WaveShaperNode group delays are
      // implementation-defined and cannot be reliably matched externally.
      // The QC SWEEP's mix_null rule is the authoritative probe; if it
      // reports residual > −80 dB this capability block stays honest and
      // the architectural fix lives in engine topology, not here.
      dryLegHasColoration: false,

      // Oversampling: the plugin declares OS on two character stages
      // (satShape 2x, ageCrush 2x) and explicitly disables it on two
      // others (crushShape, bitsShaper) for latency-match reasons —
      // a deliberate design choice, documented in-code. A future
      // aliasing_sweep rule would surface the non-OS stages honestly.
      oversamplingDeclared: true,
    },
