...
export function createNastyBeastEngineV1(ctx) {
  ...
  return {
    ...
    // Engine_V1 contract
    paramSchema,
    // ── QC capabilities declaration (see qc_backlog.md § Capability Backfill) ──
    //   categories[]    — mental-model systems this plugin touches.
    //   subcategories[] — DSP-family IDs, for QC rule dispatch.
    capabilities: {
      categories: ['Time', 'Character'],
      subcategories: ['delay-feedback', 'distortion', 'pitch-granular'],

      // Capability flags
      hasSidechain: false,     // no external SC input; all routing internal
      hasFeedback: true,       // STACK macro drives the delay feedback loop
      hasFreeze: false,
      hasLFO: true,            // chorus LFOs (2) + ping-pong LFOs (2)
      hasStereoWidth: true,    // BUTTER macro drives ping-pong stereo spread
      hasMultiband: false,
      hasLookahead: false,
      hasTruePeak: false,
      hasPitchDetector: false, // granular OLA shifter — shifts by multiplier, no pitch tracking
      hasLPC: false,
      hasFFT: false,
      hasWDF: false,
      ...
      osThresholds: null,      // fixed per-stage OS factors; no adaptive switching
      latencySamples: 0,       // dry tap is pre-chain; no PDC needed (getLatency returns 0)
      crossoverPhase: null,    // not a multiband plugin

      // Design-intent flag. Flap Jack Man uses a pre-everything dry tap:
      // the dry leg bypasses all shapers, filters, and the delay loop.
      // mix_null_series should null cleanly; NOT in the always-on-character
      // class like ManChild (no Fairchild-style in-circuit coloration).
